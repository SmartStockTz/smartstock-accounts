  ...
  template: `
    <div class="main">
      <div class="container_shops">
        ...
        <div class="shops">
          <div
            (click)="setCurrentProject(shop)"
            *ngFor="let shop of shops"
            class="ct_shop_profile"
          >
            <div matRipple class="shop_profile">
              <img
                alt="logo"
                id="{{ shop.projectId }}"
                (error)="hideImage(shop.projectId)"
                src="{{ shop?.ecommerce?.logo }}"
              />
            </div>
            <div class="shop_name">
              {{ shop.businessName }}
...
export class ChooseShopPage implements OnInit {
  ...
  async openCreateShopDialog(): Promise<void> {
    this.shopState
      .addShoPopup(this.deviceState.isSmallScreen.value, this.shopDetails)
      .then((result) => {
        if (result) {
          this.getShops();
        }
  ...
  async setCurrentProject(shop: ShopModel): Promise<void> {
    this.userDatabase
      ...
      .catch((_) => {
        this.snackBar.open(
          "Error when trying to save your current shop",
          "Ok",
          {
            duration: 3000
          }
