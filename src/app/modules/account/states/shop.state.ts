...
export class ShopState {
  ...
  currencies: BehaviorSubject<{ cc: string, symbol: string, name: string }[]> = new BehaviorSubject<any>([]);
  ...
  async addShoPopup(isSmallScreen: boolean, shopDetails: any): Promise<any> {
    if (isSmallScreen === true) {
      const dialogRef = this.sheet.open(CreateShopSheetComponent, {
        data: shopDetails,
        closeOnNavigation: true
      });
      return firstValueFrom(dialogRef.afterDismissed())
    } else {
      const dialogRef = this.dialog.open(CreateShopDialogComponent, {
        minWidth: 500,
        data: shopDetails,
        closeOnNavigation: true
      });
