...
@Component({
  ...
  template: `
    <mat-nav-list>
      ...
      <mat-list-item
        (click)="done.emit(false)"
        routerLink="/account/shops/{{ data.shop.projectId }}/ecommerce"
      >
      ...
      <mat-list-item
        (click)="done.emit(false)"
        routerLink="/account/shops/{{ data.shop.projectId }}/settings"
      >
...
export class ShopsOptionsComponent {
  ...
  async deleteShop(shop: ShopModel): Promise<void> {
    const delObs = this.dialog
      .open(ShopDeleteConfirmDialogComponent, {
        data: {
