...
@Component({
  ...
  template: `
    <mat-progress-bar mode="indeterminate" *ngIf="getSettingsProgress" color="primary"></mat-progress-bar>
    <div class="settings-container">
      <form class="settings-form" *ngIf="settingsForm" (ngSubmit)="saveSettings()" [formGroup]="settingsForm">
        <mat-form-field>
          <mat-label>Currency</mat-label>
          <mat-select formControlName="currency">
            <mat-option *ngFor="let stockCurrency of shopState.currencies | async" [value]="stockCurrency.cc">
        ...
        <div class="status-item">
          ...
          <div class="status-checker">
            <span>{{settingsForm.get('saleWithoutPrinter').value === true ? 'YES' : 'NO'}}</span>
            <mat-slide-toggle formControlName="saleWithoutPrinter"></mat-slide-toggle>
          </div>
        </div>

        <mat-form-field>
          <mat-label>Header</mat-label>
          <textarea formControlName="printerHeader"
                    placeholder="Type here..." matInput type="text" [rows]="4">
                </textarea>
        </mat-form-field>
        <mat-form-field>
          <mat-label>Footer</mat-label>
          <textarea formControlName="printerFooter"
                    placeholder="Type here..." matInput type="text" [rows]="4">
                </textarea>
        </mat-form-field>

        <button [disabled]="saveSettingProgress"
                color="primary" mat-flat-button class="save-button">
          Update Settings
...
export class SettingsGeneralComponent implements OnInit {
  ...
  async saveSettings(): Promise<void> {
    ...
    this.settingsService.updateGeneralSettings(this.settingsForm.value, this.selectedShop).then(_ => {
      ...
      this.matSnackBar.open(reason ? reason.message : 'Fails to save settingsService, try again later',
        'Ok', {
          duration: 2000
        });
    }).finally(() => {
