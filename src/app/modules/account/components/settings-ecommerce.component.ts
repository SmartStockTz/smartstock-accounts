...
@Component({
  ...
  template: `
    <mat-progress-bar *ngIf="ecommerceGetProgress" mode="indeterminate" color="primary"></mat-progress-bar>
    <div class="settings-container">
      <form class="settings-form" *ngIf="ecommerceForm" [formGroup]="ecommerceForm">
        <div class="shop-header-container">
          <img src="{{ecommerceForm.value.logo}}" class="shop-logo" alt="Logo">
          <div class="shop-details">
            ...
            <button color="primary" (click)="browseMedia('logo',$event)" mat-button>
              Upload Logo
            </button>
          </div>
        </div>
        <div class="status-item">
          ...
          <div class="status-checker">
            <span>{{ecommerceForm.get('visibility').value === true ? 'YES' : 'NO'}}</span>
            <mat-slide-toggle formControlName="visibility"></mat-slide-toggle>
          </div>
        </div>
        <div *ngIf="getVisibility()">
          <img alt="cover" class="cover" mat-card-image [src]="ecommerceForm.value.cover">
          <button color="primary" (click)="browseMedia('cover', $event)" mat-button>
            Upload Cover Photo
          </button>
        </div>
...
export class SettingsEcommerceComponent implements OnInit, OnDestroy {
  ...
  async initEcommerceForm(ecommerce: EcommerceModel): Promise<void> {
    const visibility = typeof ecommerce.visibility === 'boolean' ? ecommerce.visibility : true;
    this.ecommerceForm = this.formBuilder.group({
      ...
      about: [ecommerce.about, [Validators.nullValidator, Validators.required, Validators.minLength(20)]],
  ...
  async saveEcommerceDetails(): Promise<void> {
    if (this.getVisibility() === false || this.ecommerceForm.valid) {
      ...
      this.settingsService.updateEcommerceDetails(d, this.selectedShop).then(value => {
        this.messageService.showMobileInfoMessage('E-Commerce details updated', 2000, 'bottom');
        ...
        this.messageService.showMobileInfoMessage(reason.message, 2000, 'bottom');
      });
    } else {
